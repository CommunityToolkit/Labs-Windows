    ...
    public sealed partial class MazeSample : Page
    {
        ...
        public MazeSample()
        {
            ...
            for (int i = 0; i < MazeSize; i++)
            {
                ...
                {
                    elements.Add(CreateElement(TileType.Floor, i, j));
                    elements.Add(CreateElement(TileType.Ceiling, i, j));

                    if (j == 0)
                    {
                        elements.Add(CreateElement(TileType.HorizontalWall, i, j));
                    }
                    else if (j == MazeSize - 1)
                    {
                        elements.Add(CreateElement(TileType.HorizontalWall, i, j + 1));
                    }

                    if (i == 0)
                    {
                        elements.Add(CreateElement(TileType.VerticalWall, i, j));
                    }
                    else if (i == MazeSize - 1)
                    {
                        elements.Add(CreateElement(TileType.VerticalWall, i + 1, j));
                    }
                }
            }
        ...
        private (uint, Action<CompositionPropertySet, Dictionary<string, object>>) CreateElement(TileType type, int x, int y)
        {
            return (nextEntityId++, (_, dict) =>
            {
                dict[nameof(TileType)] = type;
                dict[X] = x;
                dict[Y] = y;
            }
            );
        ...
        public abstract class MazeLayout : Layout<uint>
        {
            ...
            {
                element.Properties.TryGetValue(X, out var x);
                element.Properties.TryGetValue(Y, out var y);
                element.Properties.TryGetValue(nameof(TileType), out var tileType);
            ...
            {
                var localOrientation = element.Properties[nameof(TileType)] switch
                {
            ...
            protected override void ConfigureElement(ElementReference<uint> element)
            {
                ...
                {
                    rect.Fill = TileControl.BrushFor((TileType)element.Properties[nameof(TileType)]!);
                }
            }
        ...
        private class TileControl
        {
            ...
            public static void LoadBrushes()
            {
                ...
                {
                    ImageSource = new BitmapImage(new Uri("ms-appx:///CompositionCollectionView.Sample/Assets/wall.bmp"))
                };

                CeilingBrush = CeilingBrush ?? new ImageBrush()
                {
                    ImageSource = new BitmapImage(new Uri("ms-appx:///CompositionCollectionView.Sample/Assets/ceiling.bmp"))
                };

                FloorBrush = FloorBrush ?? new ImageBrush()
                {
                    ImageSource = new BitmapImage(new Uri("ms-appx:///CompositionCollectionView.Sample/Assets/floor.bmp"))
                };
            }
