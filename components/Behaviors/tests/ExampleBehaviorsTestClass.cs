...
public partial class ExampleBehaviorsTestClass : VisualUITestBase
{
    ...
    {
        var assembly = typeof(Behaviors).Assembly;
        var type = assembly.GetType(typeof(Behaviors).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find Behaviors type.");
        Assert.AreEqual(typeof(Behaviors), type, "Type of Behaviors does not match expected type.");
    }
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new Behaviors_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
