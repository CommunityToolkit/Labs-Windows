...
namespace OpacityMaskViewExperiment.Tests;

[TestClass]
public partial class ExampleOpacityMaskViewTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(OpacityMaskView).Assembly;
        var type = assembly.GetType(typeof(OpacityMaskView).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find OpacityMaskView type.");
        Assert.AreEqual(typeof(OpacityMaskView), type, "Type of OpacityMaskView does not match expected type.");
    }
    ...
    {
        var component = new OpacityMaskView();
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleOpacityMaskViewTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<OpacityMaskView_ClassicBinding>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("OpacityMaskViewControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleOpacityMaskViewTestPage page)
    {
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new OpacityMaskView_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
