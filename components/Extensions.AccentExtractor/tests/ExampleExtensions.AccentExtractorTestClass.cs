...
namespace Extensions.AccentExtractorTests;

[TestClass]
public partial class ExampleExtensions.AccentExtractorTestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(Extensions.AccentExtractor).Assembly;
        var type = assembly.GetType(typeof(Extensions.AccentExtractor).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find Extensions.AccentExtractor type.");
        Assert.AreEqual(typeof(Extensions.AccentExtractor), type, "Type of Extensions.AccentExtractor does not match expected type.");
    }
    ...
    {
        var component = new Extensions.AccentExtractor();
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleExtensions.AccentExtractorTestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<Extensions.AccentExtractor_ClassicBinding>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("Extensions.AccentExtractorControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleExtensions.AccentExtractorTestPage page)
    {
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new Extensions.AccentExtractor_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
