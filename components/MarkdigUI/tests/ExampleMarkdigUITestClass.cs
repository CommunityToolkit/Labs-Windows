...
namespace MarkdigUIExperiment.Tests;

[TestClass]
public partial class ExampleMarkdigUITestClass : VisualUITestBase
{
    // If you don't need access to UI objects directly or async code, use this pattern.
    [TestMethod]
    public void SimpleSynchronousExampleTest()
    {
        var assembly = typeof(MarkdigUI).Assembly;
        var type = assembly.GetType(typeof(MarkdigUI).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find MarkdigUI type.");
        Assert.AreEqual(typeof(MarkdigUI), type, "Type of MarkdigUI does not match expected type.");
    }
    ...
    [UIThreadTestMethod]
    public void SimpleUIExamplePageTest(ExampleMarkdigUITestPage page)
    {
        // You can use the Toolkit Visual Tree helpers here to find the component by type or name:
        var component = page.FindDescendant<MarkdigUI_ClassicBinding>();

        Assert.IsNotNull(component);

        var componentByName = page.FindDescendant("MarkdigUIControl");
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleMarkdigUITestPage page)
    {
    ...
    public async Task ComplexAsyncUIExampleTest()
    {
        await EnqueueAsync(() =>
        {
            var component = new MarkdigUI_ClassicBinding();
            Assert.IsNotNull(component);
        });
    }
