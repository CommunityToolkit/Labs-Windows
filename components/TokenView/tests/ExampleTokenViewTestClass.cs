...
public partial class ExampleTokenViewTestClass : VisualUITestBase
{
    ...
    {
        var assembly = typeof(TokenView).Assembly;
        var type = assembly.GetType(typeof(TokenView).FullName ?? string.Empty);

        Assert.IsNotNull(type, "Could not find TokenView type.");
        Assert.AreEqual(typeof(TokenView), type, "Type of TokenView does not match expected type.");
    }
    ...
    [UIThreadTestMethod]
    public async Task SimpleAsyncUIExamplePageTest(ExampleTokenViewTestPage page)
    {
